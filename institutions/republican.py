...
class RepublicanModel(BaseInstitutionModel):
    ...
    def __init__(
        ...
        num_parties: int = 2,
        discipline_strength: float = 0.4,  # Much weaker than parliamentary
        num_committees: int = 3,
        committee_size: int = 5,
        committee_gatekeeping_power: float = 0.4,  # Stronger than parliamentary
        executive_opposition_rate: float = 0.3,  # How often executive opposes legislature
        ...
        # Republican-specific parameters
        self.discipline_strength = discipline_strength  # Weaker party discipline
        self.executive_opposition_rate = executive_opposition_rate
        
        # Committee system parameters (stronger than parliamentary)
        ...
        self.executive_party = None  # Party of the president
        self.president = None  # Elected executive
        self.executive_ideology = (0.0, 0.0)  # President's ideological position
        
        # Legislative state (no government formation like parliamentary)
        self.legislative_session = 1
        self.bills_passed = 0
        self.bills_vetoed = 0
        self.gridlock_events = 0
        
        # Committee system (central to republican governance)
        self.committees: List[CommitteeAgent] = []
        self.bills_in_committee: Dict[int, int] = {}  # bill_id -> committee_id
        self.committee_agenda: List[int] = []  # bill_ids scheduled for floor votes
    ...
    def _initialize_committees(self) -> None:
        ...
        # Republican committees have stronger agenda-setting powers
        jurisdictions = [
            CommitteeJurisdiction(
                name="House Ways and Means",
                policy_area="economic", 
                ideology_center=(0.3, 0.0),  # Slightly conservative economic center
                ideology_radius=0.9
            ),
            CommitteeJurisdiction(
                name="House Judiciary", 
                policy_area="justice",
                ideology_center=(0.1, -0.4),  # Moderate economics, conservative social
                ideology_radius=0.8
            ),
            CommitteeJurisdiction(
                name="House Foreign Affairs",
                policy_area="foreign",
                ideology_center=(0.0, -0.2),  # Neutral economics, slightly conservative social
                ideology_radius=0.7
            ),
            CommitteeJurisdiction(
                name="House Energy and Commerce",
                policy_area="environment",
                ideology_center=(-0.2, 0.1),  # Slightly liberal economics, moderate social
                ideology_radius=0.8
            ),
            CommitteeJurisdiction(
                name="House Education and Labor",
                policy_area="social", 
                ideology_center=(-0.4, 0.5),  # Liberal economics, liberal social
                ideology_radius=0.9
            )
        ]
    ...
    def _elect_executive(self) -> None:
        ...
        # Get party strengths for presidential election
        party_seats = {}
        party_legislators = {}
        ...
        if party_seats:
            # Presidential election - could be different from legislative majority
            # Simplified: random party weighted by strength, but with some chance for minority party
            if self.random.random() < 0.7:  # 70% chance largest party wins presidency
                self.executive_party = max(party_seats.keys(), key=lambda k: party_seats[k])
            else:  # 30% chance another party wins (divided government scenario)
                other_parties = [k for k in party_seats.keys()]
                self.executive_party = self.random.choice(other_parties)
            
            # Select president from winning party
    ...
    def _get_weak_disciplined_vote(self, legislator: LegislatorAgent, bill: Bill) -> bool:
        ...
        """
        if self.discipline_strength == 0:
            return legislator.decide_vote(bill)
        
        # Much weaker party discipline than parliamentary
        if self.random.random() < self.discipline_strength:
            # Party discipline - but even weaker effect
            if legislator.party_id is not None:
                # Find party average on this issue (simplified)
                party_legislators = [a for a in self.schedule.agents 
                                  if isinstance(a, LegislatorAgent) and a.party_id == legislator.party_id]
                
                if party_legislators:
                    # Party position is average of member ideologies
                    avg_ideology = [
                    ...
                    # Vote based on party average ideology
                    dist = sum((a - b) ** 2 for a, b in zip(avg_ideology, bill.ideology)) ** 0.5
                    return dist < 1.0
        
        # Individual vote based on personal ideology (more common than parliamentary)
        return legislator.decide_vote(bill)
    ...
    def _executive_veto_check(self, bill: Bill) -> bool:
        ...
        # Executive opposition based on ideological distance
        executive_distance = sum((a - b) ** 2 for a, b in zip(self.executive_ideology, bill.ideology)) ** 0.5
        
        # Higher distance = higher veto probability
        veto_probability = min(0.8, executive_distance / 2.0)  # Max 80% veto chance
        
        # Also factor in general opposition rate
        veto_probability = max(veto_probability, self.executive_opposition_rate)
    ...
    def pass_legislation(self, bill: Bill) -> bool:
        ...
        """
        # Stage 1: Committee gatekeeping (stronger than parliamentary)
        ...
        # Stage 2: Floor vote with weak party discipline
        votes_for = 0
        votes_against = 0
        ...
        # Stage 3: Executive veto check
        if self._executive_veto_check(bill):
            self.bills_vetoed += 1
            
            # Stage 4: Override attempt (need 2/3 majority)
            total_legislators = votes_for + votes_against
            override_threshold = int(total_legislators * 0.67) + 1
    ...
    def _route_to_committee(self, bill: Bill) -> Dict[str, Any]:
        ...
        if relevant_committee is None:
            # No committee jurisdiction - approve by default (rare in republican system)
        ...
        # Committee considers bill (with stronger gatekeeping than parliamentary)
    ...
    def step(self) -> None:
        ...
        """
        # Committees process legislation more actively than parliamentary
        if self.random.random() < 0.3:  # 30% chance of legislative activity
            test_bill = Bill(
