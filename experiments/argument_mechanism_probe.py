"""Prospective 2x2 probe of preference-intensity disclosure and speaker order."""
...
def run_argument_mechanism_probe(
    backend: ChatBackend,
    *,
    n_tasks: int = 12,
    ...
    """Run matched intensity-visibility and speaking-order conditions."""
    if n_tasks < 1:
    ...
    for repetition in range(n_tasks):
        ...
        try:
            ...
            if any(
                ...
                raise ValueError("mechanism probe baseline did not match the oracle")

            baseline_choice = collective_choice(
                model_task, model_task.initial_actions
            )
            for intensity_visible in (False, True):
                for minority_last in (False, True):
                    outcome, logs = run_model_argument_mechanism(
                        model_task,
                        backend,
                        seed=seed,
                    ...
                    actions.extend(_annotate_logs(model_task, logs, seed, backend.model))
...
def summarize_argument_probe(
    ...
    outcome_summary = (
        outcomes.groupby(
            ["intensity_visible", "minority_last", "institution"], sort=False
        )
        .agg(
    ...
    action_summary = (
        actions.groupby(["institution", "principal_group"], sort=False)
        .agg(
            n=("agent_id", "size"),
...
def _main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("results/agent_exploration/argument_mechanism_probe"),
    )
    ...
    outcomes, actions, errors = run_argument_mechanism_probe(
        backend,
        n_tasks=args.tasks,
    ...
    outcome_summary, action_summary = summarize_argument_probe(outcomes, actions)
    args.output.mkdir(parents=True, exist_ok=True)
    outcomes.to_csv(args.output / "argument_probe_outcomes.csv", index=False)
    actions.to_csv(args.output / "argument_probe_actions.csv", index=False)
    errors.to_csv(args.output / "argument_probe_errors.csv", index=False)
    outcome_summary.to_csv(
        args.output / "argument_probe_outcome_summary.csv", index=False
    )
    action_summary.to_csv(
        args.output / "argument_probe_action_summary.csv", index=False
    )
    print(
