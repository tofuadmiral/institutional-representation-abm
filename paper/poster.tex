...
\block{Finding 1: discipline as aggregation mechanism}{%
\centering
...
$0.05\%$ to $46.7\%$ under fragmentation. Rescue magnitude falls
monotonically across the four institutions with their dependence on
parliamentary-majority government formation.

\medskip
\textbf{The same ordering holds under polarisation with opposite sign.}
\texttt{no\_discipline} costs parliamentary $66$pp when bills are extreme.
Discipline aggregates votes toward the coalition preference, not the
free-vote median. Its effect flips sign with scenario.
}
...
\begin{enumerate}\setlength\itemsep{2.5mm}
    \item \textbf{Discipline is the universal vote-aggregation mechanism}
        for majority-dependent institutions. Its sign flips with scenario:
        it blocks passage under fragmentation, enables passage under
        polarisation.
    ...
    \item \textbf{Reform implication.} Weakening party discipline is an
        ...
        Whether it damages representation is an open empirical question
        this model can illuminate.
\end{enumerate}
}
...
\begin{itemize}\setlength\itemsep{1.5mm}
    ...
    \item $68$ automated tests, GitHub Actions CI
...
\block{References}{%
\normalsize
Bowler, Farrell, \& Katz (1999).
Cox \& McCubbins (1993).
Laver \& Kenny (2014).
Linz (1990).
Noble \& Schulmann (2017).
Shugart \& Carey (1992).
Str\o m (1990).
Tsebelis (2002).
